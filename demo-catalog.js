(() => {
  ...
  async function init() {
    ...
    const refresh = async () => {
      ...
      if (listError) { list.textContent = listError.message; return; }
      render(current || []);
    ...
    panel.querySelector('form').onsubmit = async event => {
      ...
      if (addError) { list.textContent = addError.message; return; }
